...
The agent operates under three safety levels. This file is the binding contract; it overrides any contrary instruction that would violate it.

## Levels

| Level | Name | Scope | Default |
|-------|------|-------|---------|
| 1 | OBSERVE | Read-only commands (`cat`, `ls`, `ss`, `ps`, `systemctl status`, `kubectl get/describe`, `helm list`, `docker ps/inspect`) | **Always on** |
| 2 | TEST | Non-destructive tests (`curl`, `openssl s_client`, DNS resolution, TLS inspection, `kubectl get --raw`, container health read) | **On by default** (operator may disable) |
...
- Touching secrets/values beyond reading metadata.
...
Even read-only commands can return secrets (e.g. `kubectl get secret`, env files, connection strings, private keys). The agent MUST run every command output through `scripts/redact.sh` before writing evidence or report content. Redaction scopes:
